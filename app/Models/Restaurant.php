...
class Restaurant extends Model
{
    ...
    protected $casts = [
        ...
        'is_petpooja_linked_store' => 'integer',
        'petpooja_store_id' => 'string'
    ];
    ...
    public function scopeMustBeOpen($query)
    {
        $awtTdate=date('Y-m-d');
        $awtTtime=date('H:i');
        $day = (int)date('w', strtotime($awtTdate));


        $pre_order_time=date('H:i');
        $resturantIds =RestaurantSchedule::where('day',$day)->whereTime('opening_time', '<', $pre_order_time)->whereTime('closing_time', '>', $pre_order_time)->select('restaurant_id')->pluck('restaurant_id')->toArray();
        return $query->whereIn('id', $resturantIds);
    }
