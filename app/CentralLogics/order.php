...
class OrderLogic
{
    ...
    public static function create_transaction($order, $received_by=false, $status = null)
    {
        ...
        $comission_on_delivery = $delivery_charge_comission_percentage * ( $order->delivery_charge / 100 );
        $comission_on_actual_delivery_fee = ($order->delivery_charge > 0) ? $comission_on_delivery : 0;

        if($order->free_delivery_by == 'admin')
        {
            $admin_subsidy = $order->delivery_charge;
        }
        ...
        try{
            OrderTransaction::insert([
                ...
                'restaurant_amount'=>$order_amount + $order->awt_order_pckg_charge - $comission_amount,
                // 'admin_commission'=>$comission_amount - $admin_subsidy,
                'admin_commission'=>$comission_amount + $order->total_tax_amount - $admin_subsidy,
                ...
                'original_delivery_charge'=>$order->awt_dm_tot_fare,
            ...
            if($order->delivery_man && !$order->restaurant->self_delivery_system){
                ...
                if ($order->delivery_man->earning == 1) {
                    $dmWallet->total_earning = $dmWallet->total_earning + $order->dm_tips + $order->delivery_charge - $comission_on_delivery;
                } else {
                    $adminWallet->total_commission_earning = $adminWallet->total_commission_earning + $order->dm_tips + $order->delivery_charge - $comission_on_delivery;
                }
            }
