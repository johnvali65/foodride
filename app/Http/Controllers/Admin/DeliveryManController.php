...
class DeliveryManController extends Controller
{
    ...
    public function preview(Request $request, $id, $tab = 'info')
    {
        ...
        } else if ($tab == 'awtTimeStamp') {
            $from = $request->query('from', null);
            $to = $request->query('to', null);
            $awttimestamp = $dm->time_logs()->when($from && $to, function ($query) use ($from, $to) {
                $query->whereBetween('date', [$from, $to]);
            })->paginate(config('default_pagination'));
            return view('admin-views.delivery-man.view.awttimestamp', compact('dm', 'awttimestamp'));
    ...
    public function get_deliverymen(Request $request)
    {
        ...
        $data = DeliveryMan::when($zone_ids, function ($query) use ($zone_ids) {
            ...
            })->limit(8)->get(['id', DB::raw('CONCAT(f_name, " ", l_name) as text')]);
