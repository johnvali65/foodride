...
use App\Models\BulkProductDiscount;
use App\Models\AwtDisBuy;
...
class AwtDisBuyController extends Controller
{
    public function index()
    {
        $awtdiscount = AwtDisBuy::all();
        $categories = Category::where(['id'=>1])->get();
        $awt_food = Food::where(['restaurant_id'=>171])->get();
        //'awt_buy_N_get_n_product_type'=>'n/a',
        return view('admin-views.awt-discount.index', compact('awtdiscount','awt_food','categories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'offer_food_id' => 'required',
            'offer_qty' => 'required|numeric|min:0',
            'applic_category_id' => 'required',
            'applic_food_id' => 'required',
            'applic_qty' => 'required|numeric|min:0',
            'restaurant_ids' => 'required'
        ]);
        

        DB::table('awt_dis_buy_tbl')->insert([
            'discount_title' => $request->title,
            'rest_id' => $request->restaurant_ids,
            'offer_cat_id' => $request->offer_category_id,
            'offer_menu_id' => $request->offer_food_id,
            'offer_qty' => $request->applic_qty,
            'rest_id' => $request->restaurant_ids,
            'applic_cat_id' => $request->applic_category_id,
            'applic_menu_id' => $request->applic_food_id,
            'min_qty' => $request->offer_qty            
            /*'created_at' => now(),
            'updated_at' => now()*/
            ]);
        
            DB::table('food')->where(['id' => $request->offer_food_id])->update(['awt_buy_N_get_n' => $request->title]);
            
            DB::table('food')->where(['id' => $request->offer_food_id])->update(['awt_buy_N_qty' => $request->offer_qty]);
            
            DB::table('food')->where(['id' => $request->offer_food_id])->update(['awt_get_n_qty' => $request->applic_qty]);
            
            DB::table('food')->where(['id' => $request->offer_food_id])->update(['awt_buy_N_get_n_product_type'=>'offered']);            
            
            DB::table('food')->where(['id' => $request->applic_food_id])->update(['awt_buy_N_get_n_product_type'=>'applicable']);            
...
/*    public function view($id)
    ...
    {
        $awtBulkProdDiscount = BulkProductDiscount::where(['bulk_product_discount_id' => $id])->first();
        // dd(json_decode($coupon->data));
        return view('admin-views.bulk-product-discount.edit', compact('awtBulkProdDiscount'));
    }
    ...
    {
       // $awtBulkProdId = BulkProductDiscount::find($request->id);
        ...
        //DB::table('bulk_product_discount_tbl')->select('')->where('');
        
        $result = DB::table('awt_dis_buy_tbl')
            ->select('offer_menu_id','applic_menu_id')
            ->where('discount_buy_id', $request->id)
            ->first();        
        
            DB::table('food')->where(['id' => $result->offer_menu_id])->update(['awt_buy_N_get_n' => 'n/a']);
            
            DB::table('food')->where(['id' => $result->offer_menu_id])->update(['awt_buy_N_qty' => 0]);
            
            DB::table('food')->where(['id' => $result->offer_menu_id])->update(['awt_get_n_qty' => 0]);
            
            DB::table('food')->where(['id' => $result->offer_menu_id])->update(['awt_buy_N_get_n_product_type'=>'n/a']);            
            
            DB::table('food')->where(['id' => $result->applic_menu_id])->update(['awt_buy_N_get_n_product_type'=>'n/a']);  
            
        
           // $newQry = DB::table('food')->where(['restaurant_id' => $awtBulkProdId->restaurant_id],['category_id' => $awtBulkProdId->category_id])->update([
             //   'discount' => 0
            //]);          
      //  dd($newQry);exit;
        $result1 = DB::table('awt_dis_buy_tbl')->where('discount_buy_id', $request->id)->delete();
