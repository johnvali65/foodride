...
class BulkProductDiscountController extends Controller
{
    public function index()
    {
        $bulkdiscounts = BulkProductDiscount::all();
        $categories = Category::where(['position' => 0])->get();
        return view('admin-views.bulk-product-discount.index', compact('bulkdiscounts','categories'));
    }
    
    public function contactus_index(){
        $bulkdiscounts = BulkProductDiscount::all();
        return view('admin-views.bulk-product-discount.contactus_index', compact('bulkdiscounts'));        
    }
    ...
    public function store(Request $request)
    {
        ...
        DB::table('bulk_product_discount_tbl')->insert([
            'restaurant_id' => $request->restaurant_ids,
            'category_id' => $request->category_id,
            'title' => $request->title,            
            'type' => $request->discount_type??'',
            'value' => $request->discount_type == 'amount' ? $request->discount : $request['discount'],
            'created_at' => now(),
            'updated_at' => now()
            ]);
        
            DB::table('food')->where(['restaurant_id' => $request->restaurant_ids],['category_id' => $request->category_id])->update([
