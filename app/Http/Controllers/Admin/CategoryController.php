...
class CategoryController extends Controller
{
    function index()
    {
        $categories = Category::orderBy('priority')->paginate(config('default_pagination'));
    ...
    function store(Request $request)
    {
        $request->validate([
            ...
            'priority.required' => translate('messages.priority is required!'),
        ]);

        $category = new Category();
        $category->name = $request->name[array_search('en', $request->lang)];
        $category->priority = $request->priority[array_search('en', $request->lang)];
