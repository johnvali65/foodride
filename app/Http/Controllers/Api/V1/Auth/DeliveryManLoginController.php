...
class DeliveryManLoginController extends Controller
{
    public function login(Request $request)
    {
        ...
        if (auth('delivery_men')->attempt($data)) {
            ...
            return response()->json(['token' => $token, 'personal_data' => $delivery_man, 'zone_wise_topic'=>$delivery_man->type=='zone_wise'?$delivery_man->zone->deliveryman_wise_topic:'restaurant_dm_'.$delivery_man->restaurant_id], 200);
        } else {
            $errors = [];
