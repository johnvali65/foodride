...
class CustomerAuthController extends Controller
{
    ...
    public function login(Request $request)
    {
       ...
       // dd($customer_verification);
        if (auth()->attempt($data)) {
            ...
            {
                if($request->phone == '9999999999' || '8688723540') {
        		    $otp_code=1234;
        		} else {
	...
	public function awtLogin(Request $request){
        ...
        // Check if the user is referred by someone using ref_code
        $referrer = null;
        $defaultPwd = "123456";

        if ($user && empty($user->social_id)) {
            $pwd = $defaultPwd;
        } else if ($user) {
            $pwd = $user->social_id;
        } else {
            $pwd = bcrypt($defaultPwd);
        }

        // If the user is new and has a ref_code in the request
        if (!$user && $request->has('ref_code')) {
            $referrer = User::where('ref_code', $request->input('ref_code'))->first();
        }

        if ($user && Auth::attempt(['phone' => $request->phone, 'password' => $pwd])) {
            $token = auth()->user()->createToken('RestaurantCustomerAuth')->accessToken;
        } else {
            ...
            if ($referrer) {
                // Set the user being registered as referred by the referrer
                $user->referred_by = $referrer->id;
                // Add the referral amount to the referrer's wallet_balance
                $refereeAmount = BusinessSetting::where('key', 'referee_earning_rate')->value('value');
                $user->referee_amount = $refereeAmount;
                $user->wallet_balance = $refereeAmount;
                $user->save();
            }
            ...
            $user->save();
            $user = User::where('phone', $request->phone)->first();
            if($request->has('ref_code') && $referrer != '') {
                CustomerLogic::create_wallet_transaction($user->id, $refereeAmount, 'add_fund', 'Referral Bonus');
            }
        ...
        if($request->phone == '9999999999' || $request->phone == '8688723540') {
            $otp_code=1234;
        }
        ...
        $awtPhoneNo = $request->phone;
        // $url = 'https://www.fast2sms.com/dev/bulkV2?';
        // $token1 = "TbT9yD2UdrBk0NMmeM0dLjhi32j9VZwGJ9gdsJsaDpSPAkX100CluHtC76EP";
        // $sender_id = "FTWSMS";

        // $query = "authorization=" . $token1 . "&route=v3&sender_id=" . $sender_id . "&message=Your%20OTP%20is%20" . $otp_code . "&language=english&flash=0&numbers=" . $awtPhoneNo;
        // $url_final = $url . '' . $query;
        // $crl = curl_init();
        // curl_setopt($crl, CURLOPT_URL, $url_final);
        // curl_setopt($crl, CURLOPT_FRESH_CONNECT, true);
        // curl_setopt($crl, CURLOPT_RETURNTRANSFER, true);
        // $response1 = curl_exec($crl);
        // curl_close($crl);
        // info("OTP Code - " . $otp_code . " Phone number - " . $awtPhoneNo);
        $message = "Dear customer,

Your OTP is ".$otp_code.". Please do not share it with anyone for security reasons.

Thanks and Regards
Food Ride";
        $template_id = 1407169268296117912;
        Helpers::send_cmoon_sms($awtPhoneNo, $message, $template_id);
        ...
        //return response()->json(['token' => $token, 'otp'=>$otp_code , 'Authorized Successfully', 'user_data'=>$user], 200);
	}

	public function awtResendLogin(Request $request){
...
Food Ride";
		...
		return response()->json(['token' => $token, 'otp'=>$otp_code , 'Authorized Successfully', 'sms_response'=>'', 'user_data'=>$user], 200);
		//return response()->json(['token' => $token, 'otp'=>$otp_code , 'Authorized Successfully', 'user_data'=>$user], 200);
	}
}
